...
void disassembleInstr(uint32_t pc, uint32_t instr) {
    ...
    opcode = instr >> 26; //Extracting most significant 6 bits for opcode 
    rs = (instr >> 21) & 0x1F; //Extracting 5 bits for register rs
    rt = (instr >> 16) & 0x1F; //Extracting 5 bits for register rt
    rd = (instr >> 11) & 0x1F; //Extracting 5 bits for register rd
    shamt = (instr >> 6) & 0x1F; //Extracting 5 bits for shift amount
    funct = instr & 0x3F; //Extracting 6 bits for function type
    uimm = instr & 0xFFFF; //Extracting 16 bit unsigned immediate 
    simm = (instr & 0x8000) >> 15 == 1 ? uimm + 0xFFFF0000 : uimm; //Extracting 16 bit signed immediate
    addr = instr & 0x3FFFFFF; //Extracting 26 bits for address
    
    cout << hex << setw(8) << pc << ": ";
    switch(opcode) {
        case 0x00:
            switch(funct) {
                case 0x00: cout << "sll " << regNames[rd] << ", " << regNames[rs] << ", " << dec << shamt;
                    break;
                case 0x03: cout << "sra " << regNames[rd] << ", " << regNames[rs] << ", " << dec << shamt;
                    break;
                case 0x08: cout << "jr " << regNames[rs];
                    break;
            ...
            break;
        case 0x02: cout << "j " << hex << ((pc + 4) & 0xf0000000) + addr * 4; 
            break;
        case 0x03: cout << "jal " << hex << ((pc + 4) & 0xf0000000) + addr * 4;
            break;
        case 0x04: cout << "beq " << regNames[rs] << ", " << regNames[rt] << ", " 
                << hex << (pc + 4) + (simm * 4);
            break;
        case 0x05: cout << "bne " << regNames[rs] << ", " << regNames[rt] << ", " 
                << hex << (pc + 4) + (simm * 4);
            break;
        case 0x09: cout << "addiu " << regNames[rt] << ", " << regNames[rs] << ", " << dec << simm;
            break;
        case 0x0c: cout << "andi " << regNames[rt] << ", " << regNames[rs] << ", " << dec << simm;
            break;
        case 0x0f: cout << "lui " << regNames[rt] << ", " << dec << simm; 
            break;
        case 0x1a: cout << "trap " << hex << addr; 
            break;
        case 0x23: cout << "lw " << regNames[rt] << ", " << dec << simm << "(" << regNames[rs] << ")"; 
            break;
        case 0x2b: cout << "sw " << regNames[rt] << ", " << dec << simm << "(" << regNames[rs] << ")";
            break;
        default: cout << "unimplemented";
    }
    cout << endl;
